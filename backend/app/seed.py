...
Inserts the controlled category set, a starter tag set, an admin + a creator, and a few skills
spanning the lifecycle so the catalog and the review queue both render on first run.
...
from app.models import Category, Skill, SkillStatus, Tag, User
...
async def _get_or_create_user(session, email, display_name, password, role) -> User:
...
async def _seed_skill(session, *, name, summary, owner, category_slug, status, install, source,
                      tags=None, featured=False, usage=None, rejection=None) -> None:
    ...
    skill.tags = await _tags(session, tags or [])
    session.add(skill)
    await session.commit()
...
async def seed() -> None:
    async with SessionLocal() as session:
        ...
        mina = await _get_or_create_user(session, "mina@example.com", "Mina Torres", "creator123", "creator")
        ...
        print(f"seed complete — admin={admin.email}, creator={mina.email}")
